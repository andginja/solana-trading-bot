...
export class BondingCurveAccount {
  ...
  getUniqueTokenPrice(): bigint {
    if (this.tokenTotalSupply === 0n) {
      return 0n;
    }
    const marketCap = this.getMarketCapSOL();
    // Multiply by 1e9 to preserve 9 decimal places of precision
    const scaleFactor = 1_000_000_000n;
    const scaledPrice = (marketCap * scaleFactor) / this.tokenTotalSupply;
